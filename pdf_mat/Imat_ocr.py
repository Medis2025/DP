import os
os.environ["CUDA_VISIBLE_DEVICES"] = "1"  # Maps CUDA:0 in code to actual CUDA:1
import json
import fitz  # PyMuPDF
...
import shutil
from sege.seg_processer import SegmentProcessor

class LayoutGridExporter:
    def __init__(self, base_dir, output_dir, config_path="seg_config.yml"):
        ...
        self.segment_processor = SegmentProcessor(config_path)
    ...
    def match_variants_in_text(self, text):
        matches = []
        patterns = self.load_variant_patterns_from_yml('/cluster/home/gw/MATRIX/regex_variants/segement/patterns/variant_patterns_ignore_sep.yml')
    ...
    def extract_and_plot_variants(self, image_np, layout, page_index, pdf_name, pdf_dir):
        ...
        for block in layout:
            label = block.type
            processed_segment, x1, y1 = self.segment_processor.apply_offset_and_padding(block, image_np)
            if processed_segment is None:
                continue

            x2 = x1 + processed_segment.shape[1]
            y2 = y1 + processed_segment.shape[0]
            ...
            try:
                ocr_result = self.segment_processor.ocr.predict(processed_segment)
...
if __name__ == "__main__":
    exporter = LayoutGridExporter(
        ...
        output_dir="/cluster/home/gw/MATRIX/regex_variants/segement/Matrix_reper/offset_out_Imat",
        config_path="/cluster/home/gw/MATRIX/regex_variants/segement/Matrix_reper/pdf_mat/sege/seg_config.yml")
